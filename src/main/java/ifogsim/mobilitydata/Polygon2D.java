...
public class Polygon2D implements Shape, Cloneable, Serializable {
    ...
    private void updatePath(float x, float y) {
        ...
        if (path == null) {
            ...
            if (y < _ymin)
                ...
                _ymax = y;
            bounds = new Rectangle2D.Float(_xmin, _ymin, _xmax - _xmin, _ymax
                    - _ymin);
        }
    }

    /**
     * Gets the polyline2 d.
     *
     * @return the polyline2 d
     */
    /*
     ...
     * of this <code>Polygon</code>. An optional {@link AffineTransform} can be
...
class Polyline2D implements Shape, Cloneable, Serializable {
     ...
     * creating a new empty <code>Polyline2D</code> might be more memory
     * efficient than resetting the current one if the number of vertices in the
     * new polyline data is significantly smaller than the number of vertices in
    ...
    /**
     * Gets the bounding box of this <code>Polyline2D</code>. The bounding box
     * is the smallest {@link Rectangle} whose sides are parallel to the x and y
     ...
     * interface, but in the case of Line2D objects it always returns false
    ...
    /**
     * Tests if the interior of this <code>Polyline2D</code> entirely contains
     * the specified set of rectangular coordinates. This method is required to
     * implement the Shape interface, but in the case of Line2D objects it
    ...
    /**
     * Tests if the interior of this <code>Polyline2D</code> entirely contains
     * the specified <code>Rectangle2D</code>. This method is required to
     * implement the Shape interface, but in the case of Line2D objects it
     * always returns false since a line contains no area.
     */
    public boolean contains(Rectangle2D r) {
    ...
    /*
     * get the associated {@link Polygon2D}. This method take care that may be
     * the last point can be equal to the first. In that case it must not be
     * included in the Polygon, as polygons declare their first point only once.
     */
    public Polygon2D getPolygon2D() {
        ...
        Point2D.Double p1 = new Point2D.Double(xpoints[npoints - 1],
                ypoints[npoints - 1]);
     ...
     * the <code>Shape</code>. Only SEG_MOVETO and SEG_LINETO, point types are
