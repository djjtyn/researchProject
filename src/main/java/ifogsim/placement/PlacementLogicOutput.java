...
public class PlacementLogicOutput {
	...
	// <deviceid, < app, list of modules to deploy > this is to remove deploying
	// same module more than once on a certain device.
	...
	// Integer indicates next device to send the placement request (-1 for finished,
	// or device id for others )
	Map<PlacementRequest, Integer> prStatus = new HashMap<>();

	public PlacementLogicOutput(Map<Integer, Map<Application, List<ModuleLaunchConfig>>> perDevice,
			Map<Integer, List<Pair<String, Integer>>> serviceDiscoveryInfo, Map<PlacementRequest, Integer> prStatus) {
